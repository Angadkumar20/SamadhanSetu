 ...
 * Closed Verified Network Workspace for Corporate & Industry Partners:
 * 1. Checks account verification from Government Administration
 * 2. Lists civic challenges assigned to or partnered with the industry
 * 3. Enables progress logs (CSR, prototyping, mentorship) and joint solution submission
 */
function IndustryDashboard() {
  ...
  const [progressPct, setProgressPct] = useState(30);
  ...
  const loadData = async () => {
    ...
    try {
      ...
      console.error('Failed to load industry workspace:', error);
      setFeedback({
        type: 'error',
        message:
  ...
  const handleAccept = async (problemId) => {
    ...
    try {
      ...
      setFeedback({
        type: 'success',
        message: res.data.message || 'Collaboration accepted by your organization.',
      });
      loadData();
    } catch (error) {
      setFeedback({
  ...
  const handleStartWork = async (problemId) => {
    ...
    try {
      ...
      setFeedback({
        type: 'success',
        message: res.data.message || 'Development started. Citizen and academic partners notified.',
      });
      loadData();
    } catch (error) {
      setFeedback({
        type: 'error',
        message: error.response?.data?.message || 'Failed to initiate work.',
      });
    } finally {
      setActionLoadingId(null);
    }
  ...
  const handleProgressSubmit = async (e) => {
    ...
    try {
      ...
      setFeedback({
        type: 'success',
        message: res.data.message || 'Progress update recorded.',
      });
      setActiveModal(null);
      setProgressMsg('');
      loadData();
  ...
  const handleSolutionSubmit = async (e) => {
    ...
    try {
      ...
      setFeedback({
        type: 'success',
        message: res.data.message || 'Solution submitted for Government review!',
      });
      setActiveModal(null);
      setSolTitle('');
      setSolDesc('');
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col justify-between">
      <Navbar role="industry" onRefresh={loadData} />
      ...
      <main className="max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8 flex-1">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-teal-100 text-teal-900 text-xs font-bold uppercase tracking-wider mb-2">
              <span>Industry R&D & CSR Workspace</span>
            </div>
            <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight">
              Assigned Industrial Collaborations
            </h1>
            <p className="text-slate-600 text-xs sm:text-sm mt-1">
              Provide technological mentorship, infrastructure, and CSR engineering to resolve Jharkhand's public challenges.
            </p>
          </div>
        ...
        {!isVerified && (
          <div className="mb-6 p-4 rounded-2xl bg-amber-50 border border-amber-200 text-amber-900 flex items-start gap-3 text-xs">
            ...
            <div>
              <p className="font-bold">Enterprise Account Under Review by Government of Jharkhand</p>
              <p className="mt-0.5 text-amber-800">
                To guarantee verified societal solutions, a Government Administrator must verify corporate credentials before collaborative assignments are unlocked.
              </p>
            </div>
          </div>
        )}
        ...
        <div className="flex items-center gap-2 border-b border-slate-200 mb-6 pb-2">
          ...
          >
            Under Review ({solutionList.length})
          </button>
          <button
          ...
          >
            Completed Impact ({solvedList.length})
          </button>
        </div>
        ...
        {isLoading ? (
          <div className="bg-white rounded-2xl p-16 text-center border border-slate-200">
            ...
            <p className="text-slate-500 text-sm font-medium">Loading industrial projects...</p>
          </div>
        ) : currentTabList.length === 0 ? (
          <div className="bg-white rounded-2xl p-12 text-center border border-dashed border-slate-200">
            <div className="w-12 h-12 bg-teal-50 text-teal-600 rounded-full flex items-center justify-center mx-auto mb-2 text-xl font-bold">
              🏭
            </div>
            <h3 className="text-sm font-bold text-slate-800">No projects in this section</h3>
            <p className="text-xs text-slate-500 mt-1 max-w-sm mx-auto">
              Problems assigned to your company by the Government Administrator will appear here.
            </p>
          </div>
        ) : (
                  ...
                  <div>
                    <div className="flex items-center justify-between gap-2 mb-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="px-2.5 py-0.5 bg-teal-50 border border-teal-200 text-teal-700 text-xs font-semibold rounded-md uppercase">
                  ...
                  <div className="pt-4 border-t border-slate-100 flex flex-wrap gap-2">
                    ...
                    {problem.status === 'solution_submitted' && (
                      <div className="w-full text-center py-2 px-3 bg-violet-50 text-violet-800 border border-violet-200 text-xs font-semibold rounded-xl">
                        ⏳ Solution Awaiting Government Sign-off
                      </div>
                    )}
              ...
              <form onSubmit={handleProgressSubmit} className="space-y-4">
                <div>
                  <label className="block text-xs font-bold text-slate-700 mb-1">
                    Industry Contribution & Progress *
                  </label>
                  <textarea
                    rows="3"
                    required
                    ...
                    placeholder="e.g. CSR hardware procured; industrial sensor units assembled and shipped to Ranchi testing site."
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 mb-1">
                    Estimated Completion ({progressPct}%)
                  </label>
                  <input
                    type="range"
                    min="0"
              ...
              <form onSubmit={handleSolutionSubmit} className="space-y-3.5">
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. Solar-Powered Smart Irrigation Micro-Grid"
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Summary of industrial engineering, machinery, or software delivered..."
                    className="w-full text-xs p-2.5 rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-violet-500"
                  ></textarea>
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Field operational manual, maintenance schedule, and warranty details..."
                    className="w-full text-xs p-2.5 rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-violet-500"
                  ></textarea>
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Metric impact on citizens, sustainability, and economic output..."
                    className="w-full text-xs p-2.5 rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-violet-500"
                  ></textarea>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1">
                      Technical Demo / Report URL (Optional)
                    </label>
                    <input
                      type="url"
                      value={solDemo}
